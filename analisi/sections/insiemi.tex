...
\begin{itemize}
	\item \bt{UNIONE}: $A \cup B$, L'unione di A e B come l'insieme 
		...
		\end{equation}
	\item \bt{INTERSEZIONE}: $A \cap B$, L'intersezione di A e B come l'insieme
		\begin{equation}
			A \cap B\ =\ \{ x \in X\ :\ x\in A\ e\ x\in B\ \}
		\end{equation}
	\item \bt{DIFFERENZA}: $A \setminus B$, che equivale a
		\begin{equation}
			A \setminus B\ =\ \{x \in X\ :\ x \in A\ e\ x \notin B\ \}
		\end{equation}
	\item \bt{COMPLEMENTARE}: L'insieme complementare di $A$ in $X$ è:
		\begin{equation}
