...
\bt{ VANTAGGI }: singola rappresentazione dell'uno, facili le 
...
\end{itemize}
Le fasi del ciclo esecutivo di un'istruzione sono: \tit{prelevare} istruzione,
\tit{interpretare} istruzione, \tit{prelevare} dati, \tit{elaborare} dati,
\tit{memorizzare} dati.\\
...
Per aumentare ulteriormente le prestazioni si è pensato di mettere in \tit{ parallelo } la
pipeline con altre pipeline \bt{ Multithreading Simultaneo}.\\
I tipi di organizzazione sono:

\begin{enumerate}
	\item \bt{ Super Scalare },\\
		È l'approccio classico che abbiamo utilizzato, si permette il
		\bt{ SMT } replicando le sorgenti di esecuzione e si unifica il tutto
		mediante una cache condivisa
	\item \bt{ Multithreading Simultaneo },\\
		Simile al super scalare, a differenza che ogni \tit{ thread } ha il proprio
	...
	\item \bt{ Multicore },\\
		Si hanno più \tit{ microprocessori } nello stesso chip
\end{enumerate}
Un processore \tit{ multicore } si contraddistingue per:
...
\end{itemize}
Nel tenere la cache di secondo livello condivisa \bt{ L2 } si possono trarre molti vantaggi:

\begin{itemize}
	\item \bt{ Riduzione dei miss } totali, è molto più semplice che molti core
		trovino la stessa risorsa
	\item \bt{ Condivisione dei dati } non replicata, i dati presenti nella
		 cache condivisa non vengono poi replicati nelle cache \tit{ private } 
	\item \bt{ Cache dinamica } se si sfruttano algoritmi di sostituzione
	\item \bt{ Mezzo di comunicazione } fra i vari core 
\end{itemize}
La differenza con una cache \bt{ L2 } \tit{ non } condivisa è che questa fornirebbe un
